...
import { PipedreamMCPService, MCPAppInfo, MCPConnection, PipedreamMCPConfig } from '@/services/PipedreamMCPService';
...
const defaultConfig: PipedreamMCPConfig = {
  ...
  clientSecret: process.env.NEXT_PUBLIC_PIPEDREAM_CLIENT_SECRET || 'your-client-secret',
  projectId: process.env.NEXT_PUBLIC_PIPEDREAM_PROJECT_ID || 'your-project-id',
  redirectUri: process.env.NEXT_PUBLIC_PIPEDREAM_REDIRECT_URI || window?.location?.origin ? `${window.location.origin}/api/oauth/callback` : 'http://localhost:3000/api/oauth/callback',
...
export function usePipedreamMCP({ 
  ...
  const [connectionStatus, setConnectionStatus] = useState<'connected' | 'disconnected' | 'connecting' | 'error' | null>(null);
  ...
  const fetchAvailableApps = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch available apps'));
      return [];
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchUserConnections = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch user connections'));
      return [];
    } finally {
      setIsLoading(false);
    }
  ...
  const updateConnectionStatus = useCallback(() => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to update connection status'));
    }
  ...
  const connectOAuthApp = useCallback(async (appIdToConnect: string) => {
    if (!userId) {
    ...
    try {
      ...
      setConnectionStatus('error');
      setError(err instanceof Error ? err : new Error('Failed to initiate OAuth flow'));
      return null;
    } finally {
  ...
  const connectWithApiKey = useCallback(async (appIdToConnect: string, apiKey: string) => {
    if (!userId) {
      setError(new Error('User not logged in'));
      return false;
    }
    ...
    try {
      ...
      const service = getMCPService();
      const connection = await service.connectWithApiKey(appIdToConnect, userId, apiKey);
      ...
      setConnectionStatus('error');
      setError(err instanceof Error ? err : new Error('Failed to connect with API key'));
      return false;
    } finally {
      setIsConnecting(false);
    }
  ...
  // Disconnect an app
  const disconnectApp = useCallback(async (appIdToDisconnect: string) => {
    if (!userId) {
      setError(new Error('User not logged in'));
      return false;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to disconnect app'));
      return false;
    } finally {
  ...
  const makeApiRequest = useCallback(async <T,>(
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to make API request'));
      return null;
    }
  }, [userId, getMCPService]);

  // Get connection status for a specific app
  const getConnectionStatusForApp = useCallback((appIdToCheck: string): MCPConnection | null => {
  ...
  const getAppById = useCallback(async (appIdToGet: string): Promise<MCPAppInfo | null> => {
    try {
      const service = getMCPService();
