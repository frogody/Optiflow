...
import { AgentOrchestratorService, Agent, Flow } from '@/services/AgentOrchestratorService';
...
export function useAgentOrchestrator({ agentId, autoLoad = true }: UseAgentOrchestratorOptions = {}) {
  ...
  const startAgent = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const updatedAgent = await orchestratorService.startAgent(selectedAgent.id, currentUser.id);
  ...
  const stopAgent = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const updatedAgent = await orchestratorService.stopAgent(selectedAgent.id);
  ...
  const createFlow = useCallback(async (flow: Omit<Flow, 'id'>) => {
    if (!selectedAgent) {
      toast.error('No agent selected');
      return null;
    }
    ...
    try {
      setIsLoading(true);
      const newFlow = await orchestratorService.createFlow(selectedAgent.id, flow);
  ...
  const getAgentContext = useCallback(async (): Promise<ModelContext | null> => {
    if (!selectedAgent) {
      toast.error('No agent selected');
      return null;
    }
    ...
    try {
      const context = await orchestratorService.getAgentContext(selectedAgent.id);
      return context || null;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to get agent context';
      setError(new Error(errorMessage));
      toast.error(errorMessage);
      return null;
    }
  ...
  const updateAgentModel = useCallback(async (modelId: string) => {
    if (!selectedAgent) {
      toast.error('No agent selected');
      return null;
    }
    ...
    try {
      setIsLoading(true);
      const updatedAgent = await orchestratorService.updateAgentModel(selectedAgent.id, modelId);
