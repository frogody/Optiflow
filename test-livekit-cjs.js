// Test LiveKit token generation using CommonJS
...
async function generateToken() {
  try {
    ...
    // Generate the token - check if it's async
    let token;
    if (at.toJwt.constructor.name === 'AsyncFunction') {
      token = await at.toJwt();
      console.log('Using async token generation');
    } else {
      token = at.toJwt();
      console.log('Using sync token generation');
    }
